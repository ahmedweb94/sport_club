...
class Project_section extends CI_Controller {

    function __construct() {
        parent::__construct(); 
        $this->load->model('Project_section_model');
    ...
    public function index(){
        is_login();
        if(CheckPermission("project_section", "own_read")){
            $this->load->view('include/header');
            $this->load->view('project_section_table');                
    ...
    public function delete($id){
        ...
        $ids = explode('-', $id);
        foreach ($ids as $id) {
            $this->Project_section_model->delete($id); 
        }
        redirect(base_url().'project_section/', 'refresh');
    }
    ...
    public function dataTable (){
        ...
        $columns = array(
         array( 'db' => 'section_id', 'dt' => 0 ),array( 'db' => 'section_title', 'dt' => 1 ),array( 'db' => 'section_images', 'dt' => 2 ),array( 'db' => 'section_id', 'dt' => 3 )
     );
        ...
        foreach ($output_arr['data'] as $key => $value) {
         ...
         if(CheckPermission($table, "all_update")){
             $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnEditRow" class="modalButtonsection1 mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="Edit"><i class="fa fa-pencil" data-id=""></i></a>';
            ...
            if($user_id==$this->user_id){
             $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnEditRow" class="modalButtonnsection1 mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="Edit"><i class="fa fa-pencil" data-id=""></i></a>';
         }
     }
     
     if(CheckPermission($table, "all_delete")){
         $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="setId('.$id.', \'project_section\')" data-target="#cnfrm_delete" title="delete"><i class="fa fa-trash-o" ></i></a>';}
...
//$dataedit['section_description'] = $this->input->post('section_description');
    ...
    $this->Project_section_model->updateRow('project_section', 'section_id', $section_id, $dataedit);
    ...
    if($this->input->post('user_type') != 'admin') {
        ...
        unset($data['submit']);
        $this->Project_section_model->insertRow('project_section', $data);
        redirect( base_url().'project_section', 'refresh');
    } else {
...
public function get_modal() {
    ...
    if($this->input->post('id')){
        $data['userData'] = getDataByid('project_section',$this->input->post('id'),'section_id'); 
