...
class Game extends CI_Controller {
    ...
    public function index(){
        is_login();
        if(CheckPermission("game", "own_read")){
            $this->load->view('include/header');
            $this->load->view('game_table');                
    ...
    public function dataTable (){
        ...
        $primaryKey = 'game_id';
        $columns = array(
         array( 'db' => 'game_id', 'dt' => 0 ),array( 'db' => 'game_title', 'dt' => 1 ),array( 'db' => 'game_id', 'dt' => 2)
     );
        ...
        foreach ($output_arr['data'] as $key => $value) {
         ...
         if(CheckPermission($table, "all_update")){
             $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnEditRow" class="modalButtongame mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="Edit"><i class="fa fa-pencil" data-id=""></i></a>';
            ...
            if($user_id==$this->user_id){
             $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnEditRow" class="modalButtonngame mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="Edit"><i class="fa fa-pencil" data-id=""></i></a>';
         }
     }
     
     if(CheckPermission($table, "all_delete")){
         $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="setId('.$id.', \'game\')" data-target="#cnfrm_delete" title="delete"><i class="fa fa-trash-o" ></i></a>';}
...
//$dataedit['game_images']=$game_images;
            $dataedit['game_title'] = $this->input->post('game_title');
//$dataedit['game_description'] = $this->input->post('game_description');

            if(isset($data['edit'])){
                unset($data['edit']);
            }
            $this->Game_model->updateRow('game', 'game_id', $game_id, $dataedit);
            ...
            if($this->input->post('user_type') != 'admin') {
                ...
                //$data['game_images'] = $game_images;
                unset($data['submit']);
                $this->Game_model->insertRow('game', $data);
                redirect( base_url().'game', 'refresh');
            } else {
    ...
    public function get_modal() {
        ...
        if($this->input->post('id')){
            $data['userData'] = getDataByid('game',$this->input->post('id'),'game_id'); 
