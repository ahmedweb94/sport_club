...
class User extends CI_Controller {
    ...
    public function registration(){
      ...
      if(setting_all('register_allowed')==1){
         if($this->input->post()) {
            $this->add_edit();
            $this->session->set_flashdata('messagePr', 'تم التسجيل بنجاح');
        } else {
            $this->load->view('include/script');
     ...
     $this->session->set_flashdata('messagePr', 'غير مسموح بتسجيل عضويات جديدة');
     redirect( base_url().'user/login', 'refresh');
 }
}
    ...
    public function auth_user($page =''){ 
      ...
      if(empty($return)) { 
         $this->session->set_flashdata('messagePr', 'بيانات غير موجودة');	
         redirect( base_url().'user/login', 'refresh');  
     } else { 
         if($return == 'not_varified') {
            $this->session->set_flashdata('messagePr', 'هذه العضوية غير مفعلة يرجى مراسلة الادارة');
            redirect( base_url().'user/login', 'refresh');
        } else {
    ...
    public function forgetpassword(){
        ...
        if($this->input->post()){
            ...
            if(isset($res[0]->users_id) && $res[0]->users_id != '') { 
                ...
                if($emm) {
                    $this->session->set_flashdata('messagePr', 'تم ارسال رابط اعادة كلمة المرور على بريد الالكترونى');
                    redirect( base_url().'user/login','refresh');
                }
            } else {    
                $this->session->set_flashdata('forgotpassword', 'هذه العضوية غير موجوده');//die;
    ...
    public function reset_password(){
        ...
        if($return){
        	$this->session->set_flashdata('messagePr', 'تم تغيير كلمة المرور بنجاح');
            redirect( base_url().'user/login', 'refresh');
        } else {
        	$this->session->set_flashdata('messagePr', 'لا يمكنك تغيير كلمة المرور');
            redirect( base_url().'user/login', 'refresh');
        }
    }
 ...
 echo json_encode($output_arr);
    ...
    if($id != '') {
            ...
            if($this->input->post('currentpassword') != '') {
                ...
                if(password_verify($this->input->post('currentpassword'), $old_row->password)){
                    if($this->input->post('password') == $this->input->post('confirmPassword')){
                        ...
                        $this->session->set_flashdata('messagePr', 'كلمة المرور و تأكيدها غير متطابقين');
                        redirect( base_url().'user/'.$redirect, 'refresh');
                    }
                } else {
                    $this->session->set_flashdata('messagePr', 'ادخل كلمة مرور صالحة');
                    redirect( base_url().'user/'.$redirect, 'refresh');
                }
            } else {
                $this->session->set_flashdata('messagePr', 'كلمة المرور مطلوبة');
                redirect( base_url().'user/'.$redirect, 'refresh');
            }
        }
        $id = $this->input->post('users_id');
        ...
        if($this->input->post('user_type') != 'admin') {
            ...
            if($checkValue==false)  {  
                $this->session->set_flashdata('messagePr', 'هذا البريد يملك عضوية بالفعل');
            ...
            if($checkValue1==false) {  
                $this->session->set_flashdata('messagePr', 'اسم العضو مسجل لدينا من قبل');
                redirect( base_url().'user/userTable', 'refresh');
            }
            $data['status'] = 'active';
...
exit;
    ...
    public function chekInvitation() {
    	if($this->input->post('code') && $this->input->post('code') != '') {
    		...
    		if(is_array($res) && !empty($res)) {
    			...
    			$this->session->set_flashdata('messagePr', 'هذا الرابط به خطأ');
    			$result['result'] = 'error';
    		}
    	}
    	echo json_encode($result);
    ...
    public function register_invited($id){
        ...
        $this->User_model->updateRow('users', 'users_id', $id, $data);
        $this->session->set_flashdata('messagePr', 'تم تسجيل العضوية بنجاح');
        redirect( base_url().'user/login', 'refresh');
    }
